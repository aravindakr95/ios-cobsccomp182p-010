...
extension LocationManager: CLLocationManagerDelegate {
    func getPlace(completion: @escaping (CLPlacemark?) -> Void) {
        ...
        if coordinates == nil {
            // Sometimes simulators did not set locations. but in actual device works perfectly.
            print("Location coordinates are not available returning mock CLLocation")
            coordinates = CLLocation(latitude: 6.931970, longitude: 79.857750)
        }
